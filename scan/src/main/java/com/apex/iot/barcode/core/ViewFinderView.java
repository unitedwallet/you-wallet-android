package com.apex.iot.barcode.core;
...
public class ViewFinderView extends View implements IViewFinder {
    ...
    private final int mDefaultTextColor = getResources().getColor(R.color.viewfinder_white);
    private final int mDefaultLaserColor = getResources().getColor(R.color.viewfinder_border);
    ...
    protected Paint mLaserTipsPaint;
    ...
    private void init() {
        ...
        mLaserTipsPaint = new Paint();
        mLaserTipsPaint.setColor(mDefaultTextColor);
        mLaserTipsPaint.setTextSize((int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 17, getResources().getDisplayMetrics()));
        mLaserTipsPaint.setStyle(Paint.Style.FILL);
        mLaserTipsPaint.setAntiAlias(true);
        //finder mask paint
    ...
    public void onDraw(Canvas canvas) {
        ...
        mLaserTipsPaint.getTextBounds(tips, 0, tips.length(), rect);
        canvas.drawText(tips, getFramingRect().left + (getFramingRect().right - getFramingRect().left - rect.width()) / 2, getFramingRect().top - 100, mLaserTipsPaint);
        if (mIsLaserEnabled) {
            drawLaser(canvas);
        }
    }
    ...
    public void drawLaser(Canvas canvas) {
        ...
        translateY += 2f;
        if (translateY >= framingRect.height() - 3) {
            translateY = 2f;
        }

        //  canvas.drawRect(framingRect.left + 2, middle - 1, framingRect.right - 1, middle + 2, mLaserPaint);
        canvas.drawRect(framingRect.left + 2, framingRect.top - 1, framingRect.right - 1, framingRect.top + 2, mLaserPaint);
    ...
    public synchronized void updateFramingRect() {
       ...
       /* LinearGradient linearGradient = new LinearGradient(   mFramingRect.left  +  ( mFramingRect.right - mFramingRect.left) /2, mFramingRect.top - 1,
                mFramingRect.left  +  ( mFramingRect.right - mFramingRect.left) /2, mFramingRect.top + 40,

                R.color.viewfinder_lase,
                R.color.endss,
